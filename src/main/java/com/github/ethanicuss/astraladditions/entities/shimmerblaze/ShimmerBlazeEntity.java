...
public class ShimmerBlazeEntity extends BlazeEntity {
    ...
    public static DefaultAttributeContainer.Builder createBlazeAttributes() {
        return HostileEntity.createHostileAttributes().add(EntityAttributes.GENERIC_ATTACK_DAMAGE, 8.0).add(EntityAttributes.GENERIC_MOVEMENT_SPEED, 0.5f).add(EntityAttributes.GENERIC_FOLLOW_RANGE, 48.0);
    }
            ...
            if (d < 4.0) {
                ...
                if (this.fireballCooldown <= 0) {
                    ...
                    if (this.fireballsFired > 1) {
                        ...
                        for (int i = 0; i < 1; ++i) {
                            GluttonyBallEntity smallFireballEntity = new GluttonyBallEntity(ModEntities.GLUTTONY_BALL, this.blaze.world);
                            smallFireballEntity.setVelocity(0, 1 , 0);
